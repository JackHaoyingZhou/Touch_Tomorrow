...
class ArmSideControl:
    ...
    def get_data(self,data):
        self.clutch = int(data.clutchLeft)
        self.E_data = float(data.jawLeft)
        self.V_data.p = pk.Vector(float(data.poseLeft[1])*-0.16,#0.19,
                                  float(data.poseLeft[0])*0.18,#0.226,
                                  float(data.poseLeft[2])*0.2)#0.25)
        mid = [float(data.poseLeft[5]),float(data.poseLeft[3])]
        if abs(mid[0]) > 1:
